#!/usr/bin/env python3
"""
Joint BAO+SN with resonant BAO modulation + per-kind BAO scales.

- SN χ²: joint_fit_resonant_rd.make_sn_chi2
- BAO χ²: DM/rd and DH/rd from flat LCDM, multiplied by:
      M(z) = 1 + A*cos(f*ln(1+z)+phi)*exp(-gamma*z)
  and optionally by per-kind scales s_DM, s_DH (multiplicative)
- Full covariance via Cholesky
- Small-wiggle bounds, gentle Gaussian priors, utilities:
    --fix-gamma-zero, --bao-only, --auto-rescale-sn

Recommended:
  PYTHONPATH=. python3 -u tools/run_joint_resonant_scaled.py \
...
# -------------------- CLI --------------------
def parse_args():
    p = argparse.ArgumentParser(description="Joint BAO+SN with resonant BAO + per-kind scales.")
    ...
    # per-kind BAO calibration scales
    p.add_argument("--fit-bao-scales", action="store_true",
                   help="Fit per-kind BAO scales s_DM, s_DH (multiplicative).")
    p.add_argument("--prior-sdm-sigma", type=float, default=0.05,
                   help="Prior sigma on s_DM around 1 (default 5%).")
    p.add_argument("--prior-sdh-sigma", type=float, default=0.05,
                   help="Prior sigma on s_DH around 1 (default 5%).")

    # utilities
    p.add_argument("--bao-only", action="store_true")
    p.add_argument("--auto-rescale-sn", action="store_true",
                   help="Numerically rescale SN covariance so full-cov χ²@θ0 ~ diag-only χ²@θ0 (readability).")
    p.add_argument("--dump-residuals", action="store_true",
                   help="Print BAO residual means/RMS by kind after fit.")
    return p.parse_args()
...
def load_sn_pack(sn_csv: str, sn_cov_csv: str, diag_only: bool):
    ...
    return {"z": z, "mu": mu, "y": mu, "C": C, "L": L,
            "R": L, "L_or_R": L, "F": L, "is_prec": False}
...
def rescale_sn_pack_to_diag(sn_pack, H0, Om, theta0):
    # Scale SN C so full-cov χ² at θ0 equals diag-only χ² at θ0 (numerical readability only)
    ...
    diag_fun = make_sn_fun(tmp, H0, Om)
...
def make_bao_fun_resonant(bao_pack, H0, Om, rd, fit_scales=False):
    ...
    def predict(theta):
        # theta: [A,f,phi,gamma,(s_DM),(s_DH)] if fit_scales else first 4 only
        A, f, phi, gamma = map(float, theta[:4])
        s_DM = float(theta[4]) if fit_scales else 1.0
        s_DH = float(theta[5]) if fit_scales else 1.0
        mod = M_res(z, A, f, phi, gamma)
        y_th = np.empty_like(y, dtype=float)
        if mDM.any(): y_th[mDM] = s_DM * mod[mDM] * dm[mDM]
        if mDH.any(): y_th[mDH] = s_DH * mod[mDH] * dh[mDH]
        return y_th
...
def make_objective(sn_fun, bao_fun, prior_A_sigma=None, prior_gamma_sigma=None,
                   ...
                   fit_scales=False, prior_sdm_sigma=0.05, prior_sdh_sigma=0.05):
    def priors(theta):
        ...
        if fit_scales:
            s_dm = float(theta[4]); s_dh = float(theta[5])
            if prior_sdm_sigma and prior_sdm_sigma > 0:
                pen += ((s_dm - 1.0) / prior_sdm_sigma) ** 2
            if prior_sdh_sigma and prior_sdh_sigma > 0:
                pen += ((s_dh - 1.0) / prior_sdh_sigma) ** 2
        return pen
...
def minimize(chi2, x0, method, max_evals, fix_gamma_zero=False, fit_scales=False):
    ...
    if method == "lbfgsb":
        # Small-wiggle box to avoid runaways
          bounds = [(0.0, 3.0),     # A up to 3
          (1.0, 6.0),     # f up to 6
          (-math.pi, math.pi),
          (0.0, 1.2)]     # gamma up to 1.2
        # and for scales (if enabled)
          bounds += [(0.7, 1.3), (0.7, 1.3)]

        if fix_gamma_zero:
            bounds[-1] = (0.0, 0.0)
        if fit_scales:
            bounds += [(0.8, 1.2), (0.8, 1.2)]  # s_DM, s_DH within ±20%
...
# --------------- Residuals dump --------------
def dump_bao_residuals(out_json, bao_csv, bao_cov, best):
    import numpy.linalg as la
    df = pd.read_csv(bao_csv)
    y = df["y_data"].to_numpy(float); z = df["z"].to_numpy(float)
    kind = df["kind"].astype(str).to_numpy()
    C = pd.read_csv(bao_cov, header=None).to_numpy(float)
    L = la.cholesky(C)
    ...
    # optional scales if present
    s_DM = best.get("s_DM", 1.0)
    s_DH = best.get("s_DH", 1.0)

    yth = np.where(kind=="DM", s_DM*mod*dm, s_DH*mod*dh)
    res = y - yth
    wres = la.solve(L, res)
...
def main():
    ...
    # Optional SN numeric rescale (readability)
    theta0 = [args.A0, args.f0, args.phi0, args.gamma0]
    if (not args.bao_only) and args.auto_rescale_sn and (not args.diag_only):
        rescale_sn_pack_to_diag(sn_pack, args.H0, args.Om, np.array(theta0, float))
    ...
    bao_fun = make_bao_fun_resonant(bao_pack, args.H0, args.Om, args.rd,
                                    fit_scales=args.fit_bao_scales)

    # Initial theta
    theta0 = [args.A0, args.f0, args.phi0, args.gamma0]
    if args.fit_bao_scales:
        theta0 += [1.0, 1.0]
    x0 = np.array(theta0, float)
    ...
    chi2 = make_objective(
        ...
        fit_scales=args.fit_bao_scales,
        prior_sdm_sigma=args.prior_sdm_sigma,
        prior_sdh_sigma=args.prior_sdh_sigma
    )

    # Optimize
    t0 = time.time()
    res = minimize(chi2, x0, args.optimizer, args.max_evals,
                   fix_gamma_zero=args.fix_gamme_zero if hasattr(args,"fix_gamme_zero") else args.fix_gamma_zero,
                   fit_scales=args.fit_bao_scales)
    t1 = time.time()

    # Unpack
    A,f,phi,gamma = map(float, res.x[:4])
    best = {"A":A, "f":f, "phi":phi, "gamma":gamma,
            "H0":args.H0, "Om":args.Om, "rd":args.rd}
    if args.fit_bao_scales:
        best["s_DM"] = float(res.x[4]); best["s_DH"] = float(res.x[5])

    # Breakdown
    chi2_sn  = float(sn_fun(res.x[:4])) if sn_fun is not None else 0.0
    chi2_bao = float(bao_fun(res.x))

    npar = 4 + (2 if args.fit_bao_scales else 0)
    ndof = int(sn_pack["C"].shape[0] * (0 if args.bao_only else 1)
               + bao_pack["C"].shape[0] - npar)
    ...
    if args.dump_residuals:
        dump_bao_residuals(p, args.bao_csv, args.bao_cov, best)
