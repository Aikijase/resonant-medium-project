#!/usr/bin/env python3
# tools/magnet_echo_v8.py
# Echo + Magnet — steeper Q vs tauM, non-saturating alignment
...
NX=96; NY=96; DT=0.01
DRIVE_STEPS=3500; RING_STEPS=3500; SAVE_EVERY=20
SEED=7

# Wave + damping
C=1.0; OMEGA0=1.50; ZETA=0.009

# Memory scaling (steeper than v7)
BETA0=0.16; TAU_REF=5.0; P_MEM=1.3
TAU_LIST=[1.0,2.0,5.0,10.0,20.0,40.0]

# Magnet (a touch more diffusion, a touch less advection)
SIGMA=0.85; GAMMA_M=0.06; D_M=0.010; ALPHA=0.12; DAMP_M=0.0
M_SOFTCLIP=200.0

# Directional drive
KX=2*np.pi/24.0; SIGMA_Y=14.0
DRIVE_AMP0=0.026
def drive_amp_for_tau(tau):          # stronger taper to keep linear
    return DRIVE_AMP0 * (tau/TAU_REF)**(-0.5)

def sweep_offsets_for_tau(tau):      # narrower sweep for larger tau
    span=0.22*(TAU_REF/tau)**0.35
    return np.linspace(-span, +span, 15)
...
def build_drive(nx,ny,kx,sig_y):
    X,Y=np.meshgrid(np.arange(nx),np.arange(ny),indexing="ij")
    cy=ny//2; gy=np.exp(-((Y-cy)**2)/(2.0*sig_y**2))
    plane=np.cos(kx*X); m=np.mean(np.abs(plane*gy))+EPS
    return (plane*gy)/m
DRIVE_MASK=build_drive(NX,NY,KX,SIGMA_Y)

def alignment_cos2(y,Mx,My,top_frac=0.15):   # tighter mask => less saturation
    gx,gy=grad(y)
...
def simulate(tau, w, amp, record=True):
    ...
    for t in range(DRIVE_STEPS):
        y += amp*np.sin(w*t*DT)*DRIVE_MASK
        ...
        y += DT*v; y*=0.999
        if record and (t%SAVE_EVERY)==0: sd.append(float(np.mean(np.abs(y))))
    # Ring-down
    for t in range(RING_STEPS):
        ...
        v += DT*(C*C*lap_y - two_z_om0*v - OMEGA0*OMEGA0*y + mem + adv)
        y += DT*v; y*=0.999
        if record and (t%SAVE_EVERY)==0: sr.append(float(np.mean(np.abs(y))))
    return (np.asarray(sd),np.asarray(sr)), (Mx,My,y), BETA_eff

def main():
    fs=1.0/(DT*SAVE_EVERY)
    rows=[]; spectra=[]; last=None
    for tau in TAU_LIST:
        amp=drive_amp_for_tau(tau)
        # Pick best drive ω for this tau
        best=None
        for d in sweep_offsets_for_tau(tau):
            w=OMEGA0*(1.0+d)
            (sd,_),_,_=simulate(tau,w,amp,record=True)
        ...
        (sd,sr),(Mx,My,y),BETA_eff=simulate(tau,w,amp,record=True)
        pf,bw,Qs,f,Pxx=spectral_peak(sd,fs)
        Qr=ringdown_Q(sr,fs)
        ...
        ax.set_title(f"Spectrum τM={tau:g} (ω*={w:.3f})")
        ...
        rows.append(dict(tauM=tau, drive_omega=w, BETA_eff=BETA_eff,
                         ...
                         alignment_cos2=align, mean_M=meanM))
        spectra.append((tau,f,Pxx)); last={"tau":tau,"Mx":Mx,"My":My,"y":y}

    df=pd.DataFrame(rows)
    csv=os.path.join(OUTDIR,"magnet_echo_summary.csv"); df.to_csv(csv,index=False)
    ...
    with PdfPages(pdf) as pp:
        ...
        for i,(tau,f,Pxx) in enumerate(spectra[2:4]):
            ...
            ax.set_title(f"Spectrum τM={tau:g}"); ax.set_xlabel("f"); ax.set_ylabel("Power")
        if last:
            tau=last["tau"]; Mx,My,y=last["Mx"],last["My"],last["y"]
        ...
        axA.set_xlabel("τM"); axA.set_ylabel("Alignment ⟨cos²θ⟩"); axA.set_title("Directional alignment (top |M|, 15%)"); axA.grid(True,alpha=0.3)
        fig.suptitle("Echo + Magnet v8: Steeper Q vs τM & Non-saturating Alignment",y=0.995,fontsize=12)
