#!/usr/bin/env python3
"""
Joint BAO+SN with resonant BAO modulation + per-kind BAO scales (locked & stable).

- SN χ²: uses your joint_fit_resonant_rd.make_sn_chi2
- BAO χ²: DM/rd & DH/rd (flat ΛCDM) * resonant M(z)
- Optional per-kind BAO scales: s_DM, s_DH ~ N(1, σ=5%)
- "Small-wiggle" bounds to avoid runaway fits
- Utilities: --bao-only, --fix-gamma-zero, --auto-rescale-sn, --dump-residuals

Recommended starting run (see bottom).
"""
...
# import SN module from repo root
...
# ---------------- CLI ----------------
def parse_args():
    p = argparse.ArgumentParser(description="Joint BAO+SN with resonant BAO + per-kind BAO scales (stable).")
    ...
    # initial θ = [A, f, phi, gamma, (sDM), (sDH)]
    ...
    p.add_argument("--auto-rescale-sn", action="store_true",
                   help="Rescale SN covariance so full-cov χ²@θ0 ≈ diag-only χ²@θ0 (readability).")
...
def load_bao_pack(bao_csv: str, bao_cov_csv: str, diag_only: bool):
    ...
    y = df["y_data"].to_numpy(float) if "y_data" in df.columns else         df.select_dtypes(include=[np.number]).iloc[:,-1].to_numpy(float)
...
def rescale_sn_pack_to_diag(sn_pack, H0, Om, theta0):
    """Numerically rescale SN C so full-cov χ² at θ0 equals diag-only χ² at θ0 (readability only)."""
...
# ------------- BAO χ² ----------------
def make_bao_fun_resonant(bao_pack, H0, Om, rd, fit_scales=True, shared=False):
    ...
    def predict(theta):
        # theta: [A,f,phi,gamma,(sDM),(sDH)] if fit_scales
        A, f, phi, gamma = map(float, theta[:4])
        if fit_scales:
            if shared:
                s_DM = s_DH = float(theta[4])
            else:
...
def make_objective(sn_fun, bao_fun, *,
                   prior_A_sigma=0.6, prior_gamma_sigma=0.30,
                   prior_f_mean=4.0, prior_f_sigma=0.6,
                   fit_scales=True, prior_sdm_sigma=0.05, prior_sdh_sigma=0.05,
                   shared=False):
    def priors(theta):
...
# -------------- minimize ---------------
def minimize(chi2, x0, method, max_evals, *, fix_gamma_zero=False, fit_scales=True, shared=False):
    import scipy.optimize as opt
    if method == "lbfgsb":
        # Small-wiggle + safe scale box
        bounds = [(0.0, 2.0),     # A
                  (1.0, 6.0),     # f
                  (-math.pi, math.pi),
                  (0.0, 1.0)]     # gamma
        ...
        if fit_scales:
            if shared:
                bounds += [(0.8, 1.2)]          # s_BAO
            else:
                bounds += [(0.8, 1.2), (0.8, 1.2)]  # s_DM, s_DH
...
def dump_bao_residuals(bao_csv, bao_cov, best):
    ...
    sDM = best.get("s_DM", 1.0); sDH = best.get("s_DH", 1.0)
...
def main():
    ...
    bao_fun = make_bao_fun_resonant(bao_pack, args.H0, args.Om, args.rd,
                                    fit_scales=args.fit_bao_scales, shared=args.shared_bao_scale)

    # initial θ
    theta = [args.A0, args.f0, args.phi0, args.gamma0]
    if args.fit_bao_scales:
        if args.shared_bao_scale:
            best['s_DM'] = best['s_DH'] = float(res.x[4])
        elif args.fit_bao_scales:
    ...
    chi2 = make_objective(
        ...
        shared=args.shared_bao_scale
    )

    # optimize
    t0 = time.time()
    res = minimize(chi2, x0, args.optimizer, args.max_evals,
                   fix_gamma_zero=args.fix_gamma_zero,
                   fit_scales=args.fit_bao_scales,
                   shared=args.shared_bao_scale)
    t1 = time.time()
