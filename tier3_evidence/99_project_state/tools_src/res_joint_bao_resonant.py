#!/usr/bin/env python3
"""
Joint BAO+SN with resonant modulation applied to BAO distances.

- SN χ²: uses joint_fit_resonant_rd.make_sn_chi2 (works in your setup).
- BAO χ²: ΛCDM distances with multiplicative resonant modulation:
      M(z) = 1 + A*cos(f*ln(1+z) + phi) * exp(-gamma*z)
      y_DM_th = M(z) * DM(z)/rd
      y_DH_th = M(z) * DH(z)/rd
- Full covariance handled via Cholesky.

Run:
  PYTHONPATH=. python3 -u tools/res_joint_bao_resonant.py \
    ...
    --out-prefix outputs/res_bao_res_v1 \
...
def parse_args():
    ...
    p.add_argument("--prior-A-sigma", type=float, default=None,
               help="If set, add ((A-0)/σ_A)^2 to χ².")
    p.add_argument("--prior-gamma-sigma", type=float, default=None,
               help="If set, add ((gamma-0)/σ_γ)^2 to χ².")
...
def M_res(z, A, f, phi, gamma):
    # multiplicative modulation; keep strictly positive via tiny floor
    delta = A * np.cos(f*np.log1p(z) + phi) * np.exp(-gamma*z)
    return np.maximum(1.0 + delta, 1e-6)
...
def priors(theta):
    ...
    if args.prior_A_sigma and args.prior_A_sigma>0:
        pen += (A/args.prior_A_sigma)**2
    if args.prior_gamma_sigma and args.prior_gamma_sigma>0:
        pen += (gamma/args.prior_gamma_sigma)**2
    return pen
...
def make_bao_fun_resonant(bao_pack, H0, Om, rd):
    ...
    def predict(theta):
        ...
        # Build prediction in the original interleaved order:
...
# then inside minimize before opt.minimize call:
if method == "lbfgsb":
    bounds = [(0.0,5.0),(0.0,8.0),(-math.pi,math.pi),(0.0,2.0)]
    if getattr(args, "fix_gamma_zero", False):
        bounds[-1] = (0.0, 0.0)  # gamma fixed
    return opt.minimize(chi2, x0, method="L-BFGS-B", bounds=bounds, options={"maxfun": max_evals})
