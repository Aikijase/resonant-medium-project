#!/usr/bin/env python3
"""
Joint BAO+SN with resonant BAO modulation + per-kind BAO scales (v2).

- SN χ² from joint_fit_resonant_rd.make_sn_chi2 (your module).
- BAO χ²: DM/rd and DH/rd (flat ΛCDM) × resonant modulation:
      M(z) = 1 + A*cos(f*ln(1+z)+phi)*exp(-gamma*z)
  and (optionally) × per-kind scales s_DM, s_DH.
- Full covariance via Cholesky.

Stability:
- Bounds (loosened): A∈[0,3], f∈[1,6], phi∈[-π,π], gamma∈[0,1.2], sDM/sDH∈[0.7,1.3]
- Priors (recommended): Aσ≈0.5, γσ≈0.25, f~N(4.2, 0.5)

Utilities:
- --fix-gamma-zero   : lock γ=0 via bounds
- --bao-only         : ignore SN term (optimize BAO+priors only)
- --auto-rescale-sn  : rescale SN covariance numerically to match diag-only χ² at θ0 (readability)
- --dump-residuals   : print BAO residual means/RMS per-kind after fit
"""
...
# Import your SN χ² implementation
...
# -------------------- CLI --------------------
def parse_args():
    p = argparse.ArgumentParser(description="Joint BAO+SN with resonant BAO + per-kind scales (v2).")
    ...
    p.add_argument("--prior-sdm-sigma", type=float, default=0.08)
    p.add_argument("--prior-sdh-sigma", type=float, default=0.08)
...
def rescale_sn_pack_to_diag(sn_pack, H0, Om, theta0):
    # numerically rescale SN C and L so full-cov χ²@θ0 == diag-only χ²@θ0
...
def make_objective(sn_fun, bao_fun, prior_A_sigma=None, prior_gamma_sigma=None,
                   ...
                   fit_scales=False, prior_sdm_sigma=0.08, prior_sdh_sigma=0.08):
    def priors(theta):
    ...
    def chi2(theta):
        ...
        if sn_fun is not None:
            base += sn_fun(theta[:4])  # SN only needs first 4
...
def minimize(chi2, x0, method, max_evals, fix_gamma_zero=False, fit_scales=False):
    ...
    if method == "lbfgsb":
        ...
        if fix_gamma_zero:
            bounds[-1] = (0.0, 0.0)
        if fit_scales:
            bounds += [(0.7, 1.3), (0.7, 1.3)]  # s_DM, s_DH
...
def dump_bao_residuals(bao_csv, bao_cov, best):
    ...
    s_DM = best.get("s_DM", 1.0); s_DH = best.get("s_DH", 1.0)
...
def main():
    ...
    # Optional SN numeric rescale for readability
    ...
    p = Path(args.out-prefix if hasattr(args,'out-prefix') else args.out_prefix).with_suffix(".json")
